...
class Config(BaseModel):
    ...
    projects_dir: Path = Field(default_factory=lambda: Path(os.getenv("PROJECTS_DIR", "./projects")))
    ...
    offline_response: str = Field(default_factory=lambda: os.getenv("OFFLINE_RESPONSE", "💻 Laptop is offline. Message queued for processing."))

# State management
class BotState(BaseModel):
    last_update_id: Optional[int] = None
    active_project: Optional[str] = None  # Single active project
...
class VoiceNotesBot:
    ...
    def get_help_message(self, user_id: str) -> str:
        ...
        lang_name = {"en": "English", "bg": "Bulgarian", "auto": "Auto-detect"}.get(self.state.language, self.state.language)
    ...
    async def new_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        project_dir = self.config.projects_dir / f"project-{project_id}-{safe_name.replace(' ', '-').lower()}"
    ...
    async def language_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🇧🇬 Bulgarian" + (" ✓" if current_lang == "bg" else ""), callback_data="lang_bg")],
            [InlineKeyboardButton("🤖 Auto-detect" + (" ✓" if current_lang == "auto" else ""), callback_data="lang_auto")]
        ]
        
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text("🌐 Select transcription language:", reply_markup=reply_markup)
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Handle deletion confirmation
            ...
            if user_id in self.state.active_sessions and self.state.active_sessions[user_id] == project_id:
        ...
        # Handle deletion cancellation
            ...
            await query.edit_message_text("✅ Deletion cancelled. Use /projects to see your projects.")
    ...
    async def handle_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix='.ogg', dir=self.config.audio_dir, delete=False) as tmp_file:
    ...
    async def process_queued_messages(self, application: Application):
        ...
        try:
            ...
            if updates:
                logger.info(f"📬 Found {len(updates)} messages in queue. Starting batch processing...")
                ...
                for idx, update in enumerate(updates, 1):
                    ...
                    if update.message and update.message.voice:
                        ...
                        logger.info(f"🎤 Processing voice message from queue (auto-detect language)")
